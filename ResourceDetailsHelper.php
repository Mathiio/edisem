...
private function fetchToolLogo($resourceId)
{
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // TEMPORARY – Relational model helper
    // Use: action=getTemplatePropertiesFull&template_id=<id>
    // Returns every property defined on the template with metadata + samples.
    // ─────────────────────────────────────────────────────────────────────────

    /**
     * Fetch all properties of a resource template with full metadata.
     *
     * For each property the result contains:
     *   - id          : property DB id
     *   - term        : "prefix:local_name"  (e.g. "dcterms:title")
     *   - label       : human label from the template slot (or vocabulary default)
     *   - data_types  : array of data type strings configured on the slot
     *   - is_required : bool — template marks the slot as required
     *   - position    : display order on the template
     *   - sample_values: up to 5 distinct non-null literal/URI values found
     *                    in the DB for items using this template
     *   - linked_templates: distinct resource_template_ids referenced via this
     *                    property (only when type is resource link)
     *
     * @param int $templateId
     * @return array
     */
    public function getTemplatePropertiesFull(int $templateId): array
    {
        // 1. Fetch template label (sanity check)
        $tmplSql = "SELECT label FROM resource_template WHERE id = ?";
        $tmplLabel = $this->conn->fetchOne($tmplSql, [$templateId]);

        if ($tmplLabel === false) {
            return ['error' => "Template $templateId not found"];
        }

        // 2. Fetch properties defined on the template
        $propSql = "
            SELECT
                rtp.id                          AS slot_id,
                rtp.position                    AS position,
                rtp.is_required                 AS is_required,
                p.label                         AS label,
                p.id                            AS property_id,
            ...
            ORDER BY rtp.position ASC
        ";

        $slots = $this->conn->fetchAllAssociative($propSql, [$templateId]);

        if (empty($slots)) {
            return ['template' => $tmplLabel, 'properties' => []];
        }

        $propertyIds = array_column($slots, 'property_id');
        $propIdsStr  = implode(',', array_map('intval', $propertyIds));

        // 3. Fetch sample literal/URI values (up to 5 per property)
        //    We look only at resources matching this template.
        $sampleSql = "
            SELECT
                v.property_id,
                v.value,
            ...
            WHERE r.resource_template_id = ?
              AND v.property_id IN ($propIdsStr)
              AND (v.value IS NOT NULL OR v.uri IS NOT NULL OR v.value_resource_id IS NOT NULL)
            LIMIT 500
        ";

        $rawValues = $this->conn->fetchAllAssociative($sampleSql, [$templateId]);

        // Group samples by property_id (keep up to 5 distinct per property)
        $sampleMap       = [];
        $linkedTemplates = []; // property_id => [template_id => count]

        foreach ($rawValues as $rv) {
            $pid = $rv['property_id'];

            // Build sample display value
            if ($rv['type'] === 'resource' && $rv['value_resource_id']) {
                // For linked resources: collect their template IDs
                if (!isset($linkedTemplates[$pid])) {
                    $linkedTemplates[$pid] = [];
                }
                $linkedTemplates[$pid][$rv['value_resource_id']] = true;
            } else {
                $display = $rv['uri'] ?? $rv['value'];
                if ($display && !isset($sampleMap[$pid][$display])) {
                    if (!isset($sampleMap[$pid])) {
                        $sampleMap[$pid] = [];
                    }
                    if (count($sampleMap[$pid]) < 5) {
                        $sampleMap[$pid][] = $display;
                    }
                }
            }
        }

        // Resolve linked resource template IDs in bulk
        $linkedResourceIds = [];
        foreach ($linkedTemplates as $pid => $resIds) {
            foreach (array_keys($resIds) as $rid) {
                $linkedResourceIds[$rid] = true;
            }
        }

        $tmplOfResource = [];
        if (!empty($linkedResourceIds)) {
            $rids = implode(',', array_map('intval', array_keys($linkedResourceIds)));
            $rtSql = "SELECT id, resource_template_id FROM resource WHERE id IN ($rids)";
            foreach ($this->conn->fetchAllAssociative($rtSql) as $row) {
                $tmplOfResource[$row['id']] = $row['resource_template_id'];
            }
        }

        // Build linked_templates per property  { template_id => count }
        $linkedTmplMap = [];
        foreach ($linkedTemplates as $pid => $resIds) {
            foreach (array_keys($resIds) as $rid) {
                $tid = $tmplOfResource[$rid] ?? null;
                if ($tid) {
                    $linkedTmplMap[$pid][$tid] = ($linkedTmplMap[$pid][$tid] ?? 0) + 1;
                }
            }
        }

        // Resolve template labels for the linked ones
        $allLinkedTmplIds = [];
        foreach ($linkedTmplMap as $byTmpl) {
            foreach (array_keys($byTmpl) as $tid) {
                $allLinkedTmplIds[$tid] = true;
            }
        }
        $tmplLabelMap = [];
        if (!empty($allLinkedTmplIds)) {
            $tids = implode(',', array_map('intval', array_keys($allLinkedTmplIds)));
            $tlSql = "SELECT id, label FROM resource_template WHERE id IN ($tids)";
            foreach ($this->conn->fetchAllAssociative($tlSql) as $row) {
                $tmplLabelMap[$row['id']] = $row['label'];
            }
        }
        ...
        foreach ($slots as $slot) {
            $pid = $slot['property_id'];

            // Infer data_types from actual values found in DB for this property
            $dataTypes = [];
            foreach ($rawValues as $rv) {
                if ((int)$rv['property_id'] === $pid && !empty($rv['type'])) {
                    $t = $rv['type'];
            ...
            $linked = [];
            if (isset($linkedTmplMap[$pid])) {
                foreach ($linkedTmplMap[$pid] as $tid => $cnt) {
                    $linked[] = [
                        'template_id' => $tid,
                        'label'       => $tmplLabelMap[$tid] ?? "Template #$tid",
                        'count'       => $cnt
                    ];
                }
                usort($linked, fn($a, $b) => $b['count'] - $a['count']);
            }

            $properties[] = [
                'id'               => $pid,
                'term'             => $slot['prefix'] . ':' . $slot['local_name'],
                'label'            => $slot['label'],
                'position'         => (int) $slot['position'],
                'is_required'      => (bool) $slot['is_required'],
                'data_types'       => $dataTypes,
                'sample_values'    => $sampleMap[$pid] ?? [],
