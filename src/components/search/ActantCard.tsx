    ...
    <div
      ...
      <div className='flex flex-col items-start justify-start gap-5'>
        <div className='flex flex-col justify-center items-start'>
          ...
          <p className='text-16 text-400 font-medium'>
        ...
        <div className="flex-col flex items-start justify-center gap-5">
          {universities.map((university, index) => (
            <div key={index} className='flex items-center justify-center gap-5'>
              ...
              />
              <p className='text-14 text-left text-400 font-extralight'>{(university.name).replace(/Université/g, 'U.').replace(/Vincennes-Saint-Denis/g, '')}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <div className='flex flex-col rounded-12 items-center justify-center p-20 gap-20 bg-300'>
      ...
      <div className='w-full flex flex-col justify-center items-center gap-5'>
        <Skeleton className='w-[65%] rounded-8'>
          <p className='text-16 text-500 font-semibold'>_</p>
        </Skeleton>
