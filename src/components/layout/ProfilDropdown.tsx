...
export const ProfilDropdown = () => {
  ...
  const displayName = useMemo(() => {
    if (userData?.firstname && userData?.lastname) // If userData exist, return full name
      return `${userData.firstname} ${userData.lastname.charAt(0)}.`
    return userData?.firstname || userData?.lastname || 'Profil'; // Return whichever is available
  }, [userData]);
  ...
  const fullName = useMemo(() => {
    ...
    return userData?.firstname || userData?.lastname || 'Utilisateur'; // Return whichever is available
  }, [userData]);
    ...
    <Dropdown>
      ...
      <DropdownTrigger className='p-3'>
        <div className='hover:bg-c3 shadow-[inset_0_0px_10px_rgba(255,255,255,0.05)] cursor-pointer bg-c2 flex flex-row rounded-8 border-2 border-c3 items-center justify-center px-15 py-10 text-16 gap-10 text-c6 transition-all ease-in-out duration-200'>
          ...
          <span className='text-16 font-normal text-c6'>
            {isAuthenticated ? displayName : 'Profil'}
          </span>
        </div>
      </DropdownTrigger>
            ...
            <DropdownSection>
              ...
              <DropdownItem key='theme' className='p-0 hover:text-c6 text-c5'>
                <button onClick={toggleThemeMode} className='flex justify-start gap-2 hover:bg-c3 items-center w-full p-2 rounded-8 transition-all ease-in-out duration-200'>
                  ...
                  <span className='text-16 text-extralight'>
              ...
              <DropdownItem key='logout' className='p-0 hover:text-c6 text-c5'>
                <button
                  ...
                  className='flex justify-start gap-2 hover:bg-c3 items-center w-full p-2 rounded-8 transition-all ease-in-out duration-200'
                >
                  <Logout size={14} />
