  ...
  return (
    <motion.div className='w-full flex flex-col gap-25' initial='hidden' animate='visible' variants={containerVariants}>
      <motion.div
        ...
        className='cursor-pointer flex flex-col bg-100 hover:bg-200 p-25 rounded-8 gap-10 transition-all ease-in-out duration-200'
        onClick={toggleExpansion}>
        <h3 className='text-16 text-400 font-medium'>{formatDate(date) + " - " + edition}</h3>
        <p
          className='text-16 text-300 font-extralight transition-all ease-in-out duration-200 gap-10'
          style={{ lineHeight: '120%', maxHeight: expanded ? 'none' : '80px', overflow: 'hidden' }}>
          {description}
        </p>
        <p className='text-16 text-500 font-medium transition-all ease-in-out duration-200'>
...
export const ConfDetailsSkeleton: React.FC = () => {
  return (
    <div className="flex w-full p-20 bg-200 rounded-14">
