          ...
          <div className='flex flex-col gap-20'>
            {loading ? 
                Array.from({ length: 4 }).map((_, index) => (
                    <MediagraphySkeleton key={index} />
                )
            ) : mediagraphies.length === 0 ? (
                <UnloadedCard />
            ) : (
                mediagraphies.map((mediagraphies, index) => (
                <MediagraphyCard key={index} mediagraphy={mediagraphies.mediagraphy} author={mediagraphies.author} date={mediagraphies.date} type={mediagraphies.type} url={mediagraphies.url}/>
              ))
            )}
          </div>
        </Scrollbar>
