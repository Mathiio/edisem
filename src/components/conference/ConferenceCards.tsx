...
export const MediaCard: React.FC<MediaCardProps> = ({ title, author, year, type }) => {
  ...
  return (
    <Link
      ...
      <div
        className={`   transition-transform-colors-opacity ${isHovered ? 'text-action' : 'text-300'}`}>
        {getIcon()}
      </div>

      <div className='w-full flex flex-col gap-10 '>
        <div className='flex-col gap-5 flex'>
          <h3 className='text-500 text-16 font-semibold'>{title}</h3>
          <p className='text-400 text-16'>{author}</p>
        </div>
        <p className='text-400 text-14'>{year}</p>
      </div>
  ...
  return (
    <div className='w-full flex flex-col justify-start rounded-12 items-start gap-10 transition-transform-colors-opacity'>
      <div className='w-full flex flex-col gap-5 '>
        <h3 className='text-500 text-16 font-semibold'>{author}</h3>
        <p className='text-400 text-16'>{content}</p>
      </div>
      <p className='text-400 text-14'>{year}</p>
    </div>
  );
  ...
  return (
    <div className='w-full flex flex-col justify-start rounded-12 items-start gap-10 transition-transform-colors-opacity'>
      <div className='w-full flex justify-start items-center gap-10'>
        <Button className='p-10 text-16 rounded-6 text-500 hover:text-500 bg-200 hover:bg-300 transition-all ease-in-out duration-200'>
          {timecode}
        </Button>
        <h3 className='text-500 text-16 font-semibold'>{author}</h3>
      </div>
  ...
  return (
    <div className='w-full lg:h-[400px] xl:h-[450px]  sm:h-[450px]  flex flex-col justify-center items-center gap-10 transition-transform-colors-opacity'>
      <div className='w-[85%] h-full flex flex-col justify-center items-center gap-10 p-25 transition-transform-colors-opacity'>
        <FileIcon size={42} className='transition-transform-colors-opacity text-400' />
        <h2 className='text-500 text-32 font-semibold'>Oups !</h2>
        <h3 className='text-500 text-16'>Aucun contenu disponible pour cette session...</h3>
        <p className='text-400 text-14 text-center'>
...
export const MiniCardCoferencier: React.FC<MiniCardCoferencierProps> = ({ Name }) => {
  ...
  return (
    <Link
      ...
      <div className='w-full flex flex-col gap-10 '>
        <p className='text-600 text-16'>{Name}</p>
      </div>
    </Link>
  );
};
