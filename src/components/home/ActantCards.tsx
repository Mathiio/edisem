  ...
  return (
    <div
      ...
      <div className='flex flex-col justify-center items-center'>
        <p className='text-16 text-center text-500 font-medium'>{firstname} {lastname}</p>
      ...
      <div className="flex-col flex items-center justify-center gap-5">
        {universities.map((university, index) => (
          <div key={index} className='flex items-center justify-center gap-5'>
            ...
            />
            <p className='text-12 text-left text-400 font-extralight'>{(university.name).replace(/Université/g, 'U.').replace(/Vincennes-Saint-Denis/g, '')}</p>
          </div>
        ))}
      </div>
    </div>
...
export const ActantSkeleton: React.FC = () => {
  return (
    <div className='flex flex-col rounded-12 items-center justify-center p-20 gap-20 bg-300'>
      ...
      <div className='w-full flex flex-col justify-center items-center'>
        <Skeleton className='w-[85%] rounded-8'>
          <p className='text-12 text-600 font-semibold'>_</p>
        </Skeleton>
      </div>
    </div>
  );
...
export const InfoCard: React.FC<InfoCardProps> = ({ name, link }) => {
  ...
  const content = (
    <div
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      className={`border-2 border-200 h-full w-full rounded-12 flex items-center justify-start p-20 gap-20 transition-transform-colors-opacity 
        ${link ? `cursor-pointer ${isHovered ? 'border-300' : 'border-200'}` : ''}`}>
      <div className='flex flex-col justify-center items-start gap-5'>
        <p className='text-14 text-500 font-regular'>{name}</p>
      </div>
    </div>
  );
  ...
  return (
    <>
      {link ? (
        <Link isExternal className='gap-10 text-600 w-full' href={link}>
          {content}
        </Link>
      ) : (
        content
...
export const InfoSkeleton: React.FC = () => {
  return (
    <div className='h-full w-full flex flex-col rounded-12 items-start justify-start p-20 gap-5 bg-300'>
