...
export const CreateView: React.FC<CreateViewProps> = ({ onCreateItem }) => {
  ...
  return (
    <div className='flex-1 w-full h-full overflow-auto bg-c1 flex flex-col items-center justify-center p-5'>
      <div className='w-full max-w-3xl'>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.15 }} className='flex flex-col gap-5'>
          {/* Titre */}
          <div className='text-center'>
            <h1 className='text-2xl text-c6 font-medium mb-1.5'>Créer un élément</h1>
            <p className='text-sm text-c4'>Sélectionnez le type d'élément que vous souhaitez créer</p>
          </div>
            ...
            {CREATE_TYPES.map((type) => {
              ...
              return (
                <motion.button
                  ...
                  className={`
                    aspect-square flex flex-col items-center justify-center gap-8 p-2.5 rounded-xl border-2 transition-all duration-200 cursor-pointer group
                    ${isSelected ? 'border-action bg-action/10' : 'border-c3 bg-c2 hover:bg-c3 hover:border-c4'}
                  `}>
                  <div className={`w-2.5 h-2.5 flex items-center justify-center ${isSelected ? 'scale-110' : 'group-hover:scale-110'} transition-transform duration-200`}>
                    <img src={type.image} alt={type.label} className='w-full h-full object-contain' />
                  </div>
                  <span className={`text-xs font-medium text-center leading-tight ${isSelected ? 'text-c6' : 'text-c6'}`}>{type.label}</span>
                </motion.button>
              );
            })}
          </div>
          ...
          <div className='flex justify-center'>
            <Button
              size='sm'
              color='primary'
              className='bg-action text-selected h-[36px] px-5 disabled:opacity-50 disabled:cursor-not-allowed'
