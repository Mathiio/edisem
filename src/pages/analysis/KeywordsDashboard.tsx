    ...
    const fetchData = async () => {
      try {
        ...
        const processItem = (item: any, typeName: string) => {
          ...
          itemKeywords = [...extractIds(item.keywords), ...extractIds(item.motcles), ...extractIds(item.concepts)];
        ...
        // --- FINALIZE METRICS ---
        const metricsArray = Array.from(keywordMap.values())
          .map((m) => ({
            ...m,
  ...
  return (
    <Layouts className='col-span-12 flex flex-col gap-50 pt-50 max-w-[1600px] mx-auto px-20'>
      <div className='flex flex-col gap-10'>
        ...
        <p className='text-18 text-c5'>Vue d'ensemble, typologie, co-occurrences et conseils d'optimisation.</p>
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3'>
        {insights.length > 0 ? (
          insights.map((insight, i) => (
            <div
              key={i}
              className={`p-20 rounded-20 border-2 flex flex-col gap-10 ${insight.type === 'warning' ? 'bg-red-900/10 border-c3' : insight.type === 'info' ? 'bg-blue-900/10 border-blue-500/30' : 'bg-green-900/10 border-green-500/30'}`}>
              <h3 className={`text-18 font-bold ${insight.type === 'warning' ? 'text-c6' : insight.type === 'info' ? 'text-c6' : 'text-c6'}`}>{insight.title}</h3>
              <p className='text-14 text-c5 leading-snug'>{insight.desc}</p>
            </div>
          ))
        ) : (
          <div className='col-span-3 p-20 rounded-20 bg-c2 border border-c3 text-center text-c5'>Mots-clés bien équilibrés.</div>
        )}
      </div>
        ...
        <div className='bg-c1 border-2 border-c3 rounded-30 p-20 flex flex-col gap-20 max-h-[600px] overflow-hidden'>
          ...
          <div className='flex flex-wrap gap-10 overflow-y-auto content-start flex-1 pr-10'>
            ...
            {metrics.filter((m) => m.count === 0).length > 100 && (
              <span className='text-c5 text-12 self-center italic'>+ {metrics.filter((m) => m.count === 0).length - 100} ...</span>
            )}
          </div>
        </div>
      </div>
          ...
          <div className='flex flex-col gap-10 overflow-y-auto pr-10'>
            {coOccurrences.map((co, idx) => (
              <div key={idx} className='flex justify-between items-center p-15 bg-c2 rounded-12 border border-c3/30'>
                ...
                <div className='px-10 py-5 bg-c3 rounded-full text-12 font-bold text-c6'>{co.count}</div>
              </div>
            ))}
          </div>
        </div>
