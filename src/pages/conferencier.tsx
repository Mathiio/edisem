...
export const Conferencier: React.FC = () => {
  ...
  const [actant, setActant] = useState<{ id: number; name: string; link: string; interventions: string; laboratories: {name: string; link: string}[]; universities: {name: string; link: string}[]; schools: {name: string; link: string}[] } | null>(null);
        ...
        <main className='mx-auto max-w-screen-2xl w-full max-w-xl grid grid-cols-10 xl:gap-75 gap-50 p-25 transition-all ease-in-out duration-200'>
          ...
          <div className='col-span-10 flex flex-col gap-100'>
            <div className='flex flex-col gap-50'>
              <Link isExternal className='gap-10 text-default-600' href={!loadingActant ? actant?.link : "#"} showAnchorIcon anchorIcon={<LinkIcon size={28}/>}>
            ...
            <div className='gap-25 flex flex-col'>
              ...
              <div className='grid grid-cols-4 grid-rows-2 gap-25'>
                {loadingConf ? Array.from({ length: 8 }).map((_, index) => <LgConfSkeleton key={index} />) : conf.map((item, index) => (
