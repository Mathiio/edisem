...
export const SimpleDetailsCard: React.FC<SimpleDetailsProps> = ({
  ...
  return (
    <motion.div className='w-full flex flex-col gap-25' initial='hidden' animate='visible' variants={containerVariants}>
      <motion.div variants={itemVariants} className='cursor-pointer flex flex-col bg-c2 hover:bg-c3 p-25 rounded-8 gap-10 transition-all duration-200' onClick={toggleExpansion}>
        {date && <h3 className='text-16 text-c5 font-medium'>{date}</h3>}
        {description && (
          <p
            className='text-16 text-c4 font-extralight transition-all duration-200 gap-10'
        ...
        <p className='text-16 text-c5 font-semibold'>{expanded ? 'affichez moins' : '...affichez plus'}</p>
      </motion.div>
    </motion.div>
  );
};
