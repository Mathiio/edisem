...
export const getLinksFromType = async (item: any, type: string): Promise<string[]> => {
    console.log(`🔍 getLinksFromType appelé pour:`, { itemId: item.id, type, item });
    ...
    switch (type) {
      ...
      case 'citation':
        ...
        console.log(`📋 Liens trouvés pour citation ${item.id}:`, links);
        break
      case 'actant':
        links = await getLinksFromActant(item);
        console.log(`📋 Liens trouvés pour actant ${item.id}:`, links);
        break
      case 'keyword':
